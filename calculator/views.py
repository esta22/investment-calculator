    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            while current_date <= end_date:
                # 해당 월의 1일 찾기
                investment_date = current_date.replace(day=1)
            ...
            if records:
                ...
                profit_amount = last_record['total_assets'] - last_record['total_investment']

                final_result = {
                    'final_stock_value': round(last_record['stock_value_krw'], 0),
                    'final_cash': round(last_record['cash_krw'], 0),
                    'final_total_assets': round(last_record['total_assets'], 0),
                    'final_total_investment': round(last_record['total_investment'], 0),
                    'final_profit_rate': round(last_record['profit_rate'], 2),
                    'final_profit_amount': round(profit_amount, 0)
                }
            else:
                final_result = None
...
def visit_statistics(request):
    ...
    # 최근 30일 통계
    recent_stats = DailyVisit.objects.order_by('-date')[:30]
    total_stats = DailyVisit.objects.aggregate(
        total_pv=Sum('page_views'),
        total_uv=Sum('unique_visits'),
        avg_pv=Avg('page_views'),
        avg_uv=Avg('unique_visits')
    )

    return render(request, 'admin/visit_stats.html', {
